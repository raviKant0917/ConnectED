...
exports.login = catchAsync(async (req, res, next) => {
    ...
    if (!email || !password) {
        return next(
            new AppError("Either email address or password is missing", 404)
        );
    }
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    if (!freshUser) {
        return next(
            new AppError("The token belonging to the user does not exists", 401)
        );
    }

    // 4)Check if password is changed by the user or not
    if (freshUser.passwordChangedAt) {
        const chngdtimestamp = parseInt(
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    if (!user) {
        return next(
            new AppError(
                `No user with email address ${req.body.email} found`,
                404
            )
        );
    }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    if (!user) {
        return next(
            new AppError(
                `Either the token ${req.params.token} is invalid or the token has expired. Please try again`,
                400
            )
        );
    }
...
exports.checkEmailExists = catchAsync(async (req, res, next) => {
    if (
        ...
        req.body.email.split("@").length <= 1 ||
        req.body.email.split("@")[1] !== "nith.ac.in"
    ) {
        return res.status(400).json({
            status: "fail",
            message:
                "Email is either invalid or does not belong to college domain",
        });
    }
